...
local function project_root_dir_discovery(bufnr, cb)
  ...
  if not bufname:match("^" .. vim.fs.joinpath "/tmp/MetadataAsSource/") then
    ...
    if root_dir then
      ...
      vim.notify("[C# LSP] failed to find root directory - LS support is disabled", vim.log.levels.ERROR)
    end
  end
end
...
local roslyn_ls_config = {
  ...
  cmd = {
    ...
    "/home/potato/roslyn/content/LanguageServer/linux-x64/Microsoft.CodeAnalysis.LanguageServer.dll",
